...
def artist_url(artist_code):
	return BASE_URL + f'/{artist_code}/mais-tocadas.html'
...
def find_artist_songs(artist_code):
	...
	with urlopen(artist_url(artist_code)) as web:
		...
		songs = html.find('ul', 'cnt-list-songs')
		for link in songs.find_all('a', 'song-name'):
...
def lyrics(url):
	...
	lyrics = html.find('div', 'cnt-letra')
...
def generate_wordcloud(artist_code):
	...
	with open(artist_file, 'r') as inf:
		all_songs = inf.read()
	all_songs = ' '.join([process_lyrics(song, stem=stem) for song in all_songs.split('\n')])
...
generate_wordcloud(CODE_EXTREMO)